...
const renderBootstrapComponent = (componentType, props) => {
  ...
  switch (componentType) {
      ...
      return (
        <Container {...restProps}>
          {Array.isArray(children) ? children.map((child, index) => (
            <RenderChildComponent key={index} component={child.component} props={child.props} />
          )) : (
            children && <RenderChildComponent component={children.component} props={children.props} />
          )}
        </Container>
      );
    case "Row":
    ...
    case "Jumbotron":
      return (
        <Container {...restProps} style={{ backgroundColor: "#007bff", color: "white", textAlign: "center", padding: "50px 0" }}>
          {Array.isArray(children) ? children.map((child, index) => (
            <RenderChildComponent key={index} component={child.component} props={child.props} />
          )) : (
            children && <RenderChildComponent component={children.component} props={children.props} />
    ...
    case "ListGroup":
      return (
        <ListGroup>
          {restProps.items && restProps.items.map((item, index) => (
...
const isBasicHTMLElement = (componentType) => {
  const basicHTMLElements = [
    "div", "span", "p", "h1", "h2", "h3", "h4", "h5", "h6", "ul", "ol", "li", "a", "img", "button", "input", "textarea", "form", "label", "table", "thead", "tbody", "tr", "th", "td", "header", "footer", "section", "article", "nav", "aside", "main", "figure", "figcaption", "blockquote", "pre", "code", "em", "strong", "small", "sub", "sup", "i", "b", "u", "mark", "del", "ins", "s", "q", "cite", "abbr", "address", "time", "progress", "meter", "audio", "video", "canvas", "svg", "path", "circle", "rect", "line", "polyline", "polygon", "text", "g", "defs", "symbol", "use", "clipPath", "filter", "feGaussianBlur", "feOffset", "feBlend", "feColorMatrix", "feComponentTransfer", "feComposite", "feConvolveMatrix", "feDiffuseLighting", "feDisplacementMap", "feFlood", "feImage", "feMerge", "feMorphology", "feSpecularLighting", "feTile", "feTurbulence"
  ];
  return basicHTMLElements.includes(componentType);
};
